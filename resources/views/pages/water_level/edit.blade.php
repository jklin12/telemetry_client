...
<div class="panel panel-inverse">
    <div class="panel-body">
        ...
        <form action="{{ route('water_level.update')}}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="form-group row m-b-15">
                ...
                <div class="col-md-9">
                    <input type="text" name="water_level_date" id="input_water_level_date" class="form-control m-b-5 datetimepicker_input" placeholder="Masukan Tanggal" value="{{$date}}" />
                </div>
            </div>
            <table id="data-table-fixed-header" class="table table-striped table-bordered table-td-valign-middle">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $key  }}</td>
                        <td> <input type="text" name="water_level_hight[{{ $key  }}]" id="input_water_level_hight" class="form-control " placeholder="Masukan Ketinggian Air " value="{{ isset($data[$key]) ? $data[$key] : '0' }}"></td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
...
<script>
    ...
    $(".default-select2").val(<?= $station?>).trigger('change');
