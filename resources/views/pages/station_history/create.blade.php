...
<div class="panel panel-inverse">
    <div class="panel-body">
        ...
        <form action="{{ route('station_history.store')}}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group row m-b-15">
                ...
                <div class="col-md-9">
                    ...
                    <input type="text" name="{{ $kf }}" id="input_{{ $kf }}" class="form-control m-b-5" placeholder="Masukan {{ $vf['label'] }}" value="{{ old($kf) ??  '' }}">
                    @elseif($vf['form_type'] == 'password')
                    <input type="password" name="{{ $kf }}" id="input_{{ $kf }}" class="form-control m-b-5" placeholder="Masukan {{ $vf['label'] }}" value="{{ old($kf) ?? '' }}">
                    @elseif($vf['form_type'] == 'year')
                    <div class="input-group date" id="year-picker">
                        <input type="text" class="form-control" name="{{ $kf }}" id="input_{{ $kf }}" value="{{ old($kf) ??  '' }}" />
                    ...
                    <textarea class="form-control" name="{{ $kf }}" id="input_{{ $kf }}" rows="3">{{ old($kf) ??  '' }}</textarea>
...
<script>
    ...
    $('#input_station').change(function() {
        $.ajax({
            url: "<?php echo route('station_assets.ajax', '') ?>" + '/' + $(this).val(),
            success: function(data) {
                $('#input_assets').empty().trigger("change");
                var json = JSON.parse(data)
                $.each(json, function(index, value) {
                    var newOption = new Option(value.asset_name, value.assets_id, true, true);
                    // Append it to the select
                    $('#input_assets').append(newOption);
                })
